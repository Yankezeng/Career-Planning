...
class ReportService:
    ...
    def check_report(self, report_id: int) -> dict:
        report = self.get_report(report_id)
            ...
            items.append(
                {
                    ...
                    "passed": passed,
                    "message": "内容完整" if passed else "内容不足，请补充可执行细节",
                }
            )
        ...
        items.append({"key": "top_jobs", "label": "岗位匹配结果", "passed": len(top_jobs) >= 3, "message": "已包含岗位匹配结果" if len(top_jobs) >= 3 else "至少保留 3 个匹配岗位"})
        items.append({"key": "action_tasks", "label": "行动计划", "passed": len(tasks) >= 3, "message": "已包含阶段任务" if len(tasks) >= 3 else "至少保留 3 个阶段任务"})
        trend_passed = bool(trend.get("industry_demand") and trend.get("job_heat") and trend.get("skill_gap_trend"))
        items.append({"key": "trend", "label": "行业需求与趋势", "passed": trend_passed, "message": "趋势数据完整" if trend_passed else "缺少行业需求/岗位热度/能力缺口趋势"})
    ...
    def ensure_export_ready(self, report_id: int):
        result = self.check_report(report_id)
        if not result["passed"]:
            raise ValueError("报告完整性检查未通过，暂不可导出")
        return result
    ...
    def _build_report_content(self, student: Student, profile: dict, selected_match: dict, matches: list[dict], career_path: dict, summary: dict, trend: dict) -> dict:
        profile_dimensions = (profile.get("raw_metrics") or {}).get("dimension_scores") or []
        sections = [
            {"key": "exploration_match", "title": "一、职业探索与岗位匹配", "content": f"目标岗位为 {selected_match['job']['name']}，综合匹配度 {selected_match['total_score']} 分，评估覆盖基础要求、职业技能、职业素养和发展潜力四个维度。", "highlights": [f"{item['label']} {item['score']} 分" for item in selected_match.get("dimension_scores", [])]},
            {"key": "goal_and_path", "title": "二、职业目标设定与职业路径规划", "content": f"结合岗位匹配与个人画像，建议围绕 {selected_match['job']['name']} 持续推进，并沿垂直晋升路径累积岗位证据。", "highlights": [f"{item.get('level')}：{item.get('job_name')}" for item in career_path.get("vertical_path", [])]},
            {"key": "action_plan", "title": "三、行动计划与成果展示", "content": "已生成短期和中期行动任务，建议按双周检查、月度复盘节奏推进，并持续沉淀项目与实习成果。", "highlights": [f"[{task.get('stage_label')}] {task.get('title')}" for task in career_path.get("tasks", [])[:8]]},
            {"key": "industry_trend", "title": "四、行业趋势与社会需求", "content": trend.get("summary", ""), "highlights": [f"行业需求指数：{trend.get('industry_demand', {}).get('demand_index', 0)}", *[f"岗位热度：{item['job_name']}({item['value']})" for item in trend.get("job_heat", [])[:3]]]},
            {"key": "edit_export", "title": "五、编辑优化与导出", "content": "报告支持编辑、润色、完整性检查、版本回溯和导出前检查，确保输出内容可直接交付。", "highlights": ["支持报告编辑器", "支持版本保存", "支持智能润色", "支持导出前检查"]},
        ]
        return {
            "title": f"{student.name}职业规划报告",
            "summary": summary["summary"],
    ...
    def _build_industry_trend(self, job_id: int) -> dict:
        ...
        return {
            ...
            "industry_demand": {
                ...
                "industry_job_count": industry_job_count,
                "category_job_count": category_job_count,
    ...
    def _save_version(self, report: Report, action: str):
        next_version = ((self.db.query(func.max(ReportVersion.version_no)).filter(ReportVersion.report_id == report.id).scalar() or 0) + 1)
        self.db.add(
            ReportVersion(
                report_id=report.id,
    ...
    def _render_html(self, content: dict) -> str:
        return self.jinja.get_template("report_template_clean.html").render(report=content)
    ...
    def _build_pdf_sections(content: dict):
        ...
        return [
            ...
            "五、行业趋势与社会需求\n" + f"需求指数：{(trend.get('industry_demand') or {}).get('demand_index', 0)}; " + "；".join([f"{item['job_name']}({item['value']})" for item in trend.get("job_heat", [])[:5]]),
